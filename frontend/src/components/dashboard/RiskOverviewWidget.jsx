...
import { ShieldAlert, AlertTriangle, CheckCircle, ArrowRight, TrendingUp, Copy, Clock, Sparkles } from 'lucide-react';
import { Link } from 'react-router-dom';
import { formatNumber } from '../../utils/formatters';

/**
 * RiskOverviewWidget Component
 * High, Medium, Low risk counts with distribution bar, contract flag breakdown, and quick triage action.
 */
export const RiskOverviewWidget = ({ kpis }) => {
  ...
  // Contract flag statistics
  const costOutliers = kpis.costOutliers || 0;
  const duplicates = kpis.duplicates || 0;
  const delayed = kpis.delayRiskWorks || 0;
  const fundMismatches = kpis.fundMismatches || 0;
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-5 shadow-sm flex flex-col justify-between">
      <div>
        ...
        <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800">
          <div>
            <h3 className="text-base font-bold text-slate-900 dark:text-slate-100 font-display flex items-center gap-2">
              ...
              <span>AI Risk & Anomaly Portfolio</span>
            </h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
              ML contract-evaluated across {formatNumber(total)} active scheme works
            </p>
          </div>
          <span className="px-2.5 py-0.5 rounded-full text-xs font-bold bg-red-100 text-red-800 dark:bg-red-950 dark:text-red-300 font-mono">
            {high} Critical
          </span>
        </div>
        ...
        <div className="mt-4">
          <div className="flex justify-between text-xs text-slate-700 dark:text-slate-300 mb-1.5 font-medium">
            ...
            <span className="text-emerald-600 font-semibold">{compPct}% Compliant</span>
          </div>
        ...
        <div className="mt-4 grid grid-cols-3 gap-2.5">
          {/* High Risk */}
          <div className="p-3 rounded-xl border border-red-200 dark:border-red-900/60 bg-red-50/50 dark:bg-red-950/20 text-center">
          ...
          {/* Compliant */}
          <div className="p-3 rounded-xl border border-emerald-200 dark:border-emerald-900/60 bg-emerald-50/50 dark:bg-emerald-950/20 text-center">
        ...
        <div className="mt-4 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700">
          <div className="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-2 flex items-center gap-1.5">
            ...
            <span>Risk-Score Contract Flags Detected</span>
          </div>
          <div className="grid grid-cols-2 gap-2 text-xs">
            <div className="flex items-center justify-between p-2 rounded-lg bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800">
              <span className="flex items-center gap-1 text-red-600 dark:text-red-400 font-medium text-[11px]">
                <TrendingUp className="w-3 h-3" /> Cost Outlier
              </span>
              <span className="font-mono font-bold text-slate-900 dark:text-white">{costOutliers}</span>
            </div>

            <div className="flex items-center justify-between p-2 rounded-lg bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800">
              <span className="flex items-center gap-1 text-purple-600 dark:text-purple-400 font-medium text-[11px]">
                <Copy className="w-3 h-3" /> Duplicate
              </span>
              <span className="font-mono font-bold text-slate-900 dark:text-white">{duplicates}</span>
            </div>
            ...
            <div className="flex items-center justify-between p-2 rounded-lg bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800">
              ...
              <span className="font-mono font-bold text-slate-900 dark:text-white">{delayed}</span>
            </div>

            <div className="flex items-center justify-between p-2 rounded-lg bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800">
              <span className="flex items-center gap-1 text-rose-600 dark:text-rose-400 font-medium text-[11px]">
                <AlertTriangle className="w-3 h-3" /> Fund Mismatch
              </span>
              <span className="font-mono font-bold text-slate-900 dark:text-white">{fundMismatches}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800">
        <Link
          to="/alerts"
          className="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl bg-blue-600 hover:bg-blue-700 text-xs font-bold text-white shadow-sm transition-all"
        >
          <span>Open Risk Score & Anomaly Explorer</span>
